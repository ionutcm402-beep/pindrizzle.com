...
function friendlyError(error: unknown, fallback: string) {
  ...
  if (/only a link|only a phone|only a.*email/i.test(message)) return message;
  if (/location/i.test(message) && /unavailable|unsupported/i.test(message)) return "Your location could not be used for Local Chat. Try enabling location again.";
  return fallback;
}
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    void createClient().rpc("my_chat_moderation_notices", { result_limit: 5 }).then(({ data, error: noticeError }) => {
      if (cancelled || noticeError) return;
      setNotices((data || []) as ModerationNotice[]);
    });
  ...
  const loadInitial = useCallback(async (announceFailure = true) => {
    ...
    try {
      ...
      if (announceFailure) setError("Local Chat could not load nearby messages. Try again.");
    } finally {
      setLoading(false);
    }
  ...
  const loadOlder = useCallback(async () => {
    ...
    try {
      ...
      window.requestAnimationFrame(() => window.requestAnimationFrame(() => {
        ...
        if (!next) return;
        next.scrollTop = next.scrollHeight - oldHeight + oldTop;
      }));
    } catch (loadError) {
  ...
  useEffect(() => {
    ...
    const refreshChangedMessage = async (payload: RealtimePayload) => {
      ...
      try {
        ...
        setMessages((current) => {
          const already = current.some((message) => message.id === changedId);
          if (!changed) return current.filter((message) => message.id !== changedId);
          if (!already) added = changed;
          return mergeMessages(current, [changed]);
        });
        if (added) {
          setLiveAnnouncement(`${added.display_name} posted a new Local Chat message.`);
  ...
  const requestLocation = useCallback(async () => {
    ...
    setLocationState(result.state);
    setCoordinates(result.coordinates);
    if (!result.coordinates) setError(result.state === "denied" ? "Location is off. Allow location for Pindrizzle in your browser settings, then try again." : "Your location could not be found. Try again.");
  }, []);
  ...
  const send = async () => {
    ...
    try {
      ...
      setFeedback(friendlyError(sendError, "Your message could not be sent. Try again."));
    } finally {
      setSending(false);
    }
  };
  ...
  const submitReport = async () => {
    ...
    try {
      const { error: reportError } = await createClient().rpc("report_chat_message", {
        target_chat_message_id: reportTarget.id,
        report_reason: reportReason,
        report_details: reportDetails.trim(),
      });
      if (reportError) throw reportError;
      setMessages((current) => current.filter((message) => message.id !== reportTarget.id));
  ...
  const blockUser = async (message: ChatMessage) => {
    ...
    setFeedback("");
    try {
      const { data, error: blockError } = await createClient().rpc("toggle_block_user", { target_user_id: message.author_id });
  ...
  const saveEdit = async (message: ChatMessage) => {
    ...
    setEditBusy(true);
    try {
      const { error: editError } = await createClient().from("chat_messages").update({ body }).eq("id", message.id).eq("author_id", userId);
      if (editError) throw editError;
      setMessages((current) => current.map((item) => item.id === message.id ? { ...item, body, edited_at: new Date().toISOString() } : item));
      setEditingId(null);
      setEditBody("");
  ...
  const deleteOwn = async (message: ChatMessage) => {
    ...
    try {
      const { error: deleteError } = await createClient().from("chat_messages").update({ deleted_at: new Date().toISOString() }).eq("id", message.id).eq("author_id", userId);
  ...
  const renderedMessages = useMemo(() => messages.map((message, index) => {
    ...
    const grouped = Boolean(previous && previous.author_id === message.author_id && new Date(message.created_at).getTime() - new Date(previous.created_at).getTime() <= GROUP_WINDOW_MS);
    const mine = message.author_id === userId;
    const recentOwn = mine && Date.now() - new Date(message.created_at).getTime() <= GROUP_WINDOW_MS;
    return { message, grouped, mine, recentOwn };
  }), [messages, userId, clock]);
    ...
    <main className="chat-screen">
      ...
      <section className="chat-intro">
        ...
        <label className="chat-radius">
          ...
          <select value={radius} onChange={(event) => writePingRadius(Number(event.target.value) as Radius)} aria-label="Local Chat radius">
            {([0.5, 1, 3, 5] as Radius[]).map((value) => <option key={value} value={value}>{value} mi</option>)}
          </select>
        </label>
      </section>
      ...
      {!authReady ? (
        ...
        <section className="chat-room" aria-label={`Local Chat within ${radiusLabel(radius)}`}>
          ...
          {feedback && <div className="chat-feedback" role="status">{feedback}<button type="button" onClick={() => setFeedback("")} aria-label="Dismiss message">×</button></div>}
            ...
            {loading && !messages.length ? (
              ...
              <article key={message.id} className={`chat-message${mine ? " mine" : ""}${grouped ? " grouped" : ""}`}>
                ...
                <div className="chat-message-content">
                  {!grouped && <div className="chat-message-meta"><strong>{mine ? "You" : message.display_name}</strong><span>{relativeTime(message.created_at, clock)}{message.edited_at ? " · edited" : ""}</span></div>}
                  {editingId === message.id ? (
                    <div className="chat-edit-box">
                      ...
                      <div><small>{editBody.length}/{MAX_MESSAGE_LENGTH}</small><button type="button" onClick={() => { setEditingId(null); setEditBody(""); }}>Cancel</button><button type="button" onClick={() => void saveEdit(message)} disabled={editBusy || !editBody.trim()}>{editBusy ? "Saving…" : "Save"}</button></div>
                    </div>
                  ) : <p>{message.body}</p>}
                  {editingId !== message.id && (
                    <div className="chat-message-actions">
                      {mine ? recentOwn ? <><button type="button" onClick={() => { setEditingId(message.id); setEditBody(message.body); }}>Edit</button><button type="button" onClick={() => void deleteOwn(message)}>Remove</button></> : null : <><button type="button" onClick={() => { setReportTarget(message); setReportReason("spam"); setReportDetails(""); }}>Report</button><button type="button" onClick={() => void blockUser(message)}>Block user</button></>}
                    </div>
                  )}
                </div>
              </article>
      ...
      {reportTarget && (
        <div className="chat-report-backdrop" role="dialog" aria-modal="true" aria-labelledby="chat-report-title" onMouseDown={(event) => event.target === event.currentTarget && !reportBusy && setReportTarget(null)}>
          <section className="chat-report-sheet">
            <header><button type="button" onClick={() => setReportTarget(null)} disabled={reportBusy}>Cancel</button><strong id="chat-report-title">Report message</strong><span /></header>
            <p>Report “{reportTarget.body.slice(0, 120)}{reportTarget.body.length > 120 ? "…" : ""}” from {reportTarget.display_name}.</p>
            <label>Reason<select value={reportReason} onChange={(event) => setReportReason(event.target.value as ReportReason)}>{REPORT_REASONS.map((reason) => <option key={reason.value} value={reason.value}>{reason.label}</option>)}</select></label>
            {reportReason === "csam" && <div className="chat-csam-warning"><strong>Priority child-safety report</strong><span>Use this category only for suspected child sexual abuse material or exploitation. It is surfaced for immediate human review.</span></div>}
            <label>Extra context <span>optional</span><textarea value={reportDetails} maxLength={500} onChange={(event) => setReportDetails(event.target.value)} placeholder="What should the moderator know?" /></label>
      ...
      <style jsx global>{`
        ...
        .chat-intro{display:flex;align-items:flex-end;justify-content:space-between;gap:24px;margin-bottom:18px;padding:20px 22px;border:1px solid var(--site-border,#e3e8ee);border-radius:18px;background:#fff}
        .chat-intro>div{min-width:0}.chat-intro>div>span{display:block;color:#0b7187;font-size:10px;font-weight:850;letter-spacing:.11em}.chat-intro h1{margin:6px 0 7px;font-size:30px;line-height:1.05;letter-spacing:-.04em;text-align:left}.chat-intro p{max-width:620px;margin:0;color:#617080;font-size:13px;line-height:1.55;text-align:left}
        .chat-radius{display:grid;gap:5px;flex:0 0 auto}.chat-radius>span{font-size:10px;font-weight:800;color:#607080}.chat-radius select{min-width:105px;min-height:44px;border:1px solid var(--site-border,#e3e8ee);border-radius:10px;background:#f7f9fb;padding:0 10px;color:var(--site-text,#10202f);font-weight:750}
        ...
        .chat-room{height:min(72vh,760px);min-height:560px;display:grid;grid-template-rows:auto auto minmax(0,1fr) auto;overflow:hidden;border:1px solid var(--site-border,#e3e8ee);border-radius:20px;background:#fff;box-shadow:0 12px 34px rgba(8,47,74,.07)}
        .chat-room-head{min-height:54px;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:0 18px;border-bottom:1px solid #edf0f3}.chat-room-head>div{display:flex;align-items:center;gap:8px}.chat-room-head strong{font-size:13px}.chat-room-head small{color:#71808e;font-size:9px}.chat-live-dot{width:8px;height:8px;border-radius:50%;background:#25bdc8;box-shadow:0 0 0 4px rgba(37,189,200,.11)}
        ...
        .chat-message-list{min-height:0;overflow-y:auto;overscroll-behavior:contain;padding:14px 18px 18px;scrollbar-gutter:stable;-webkit-overflow-scrolling:touch}
        .chat-history-status{text-align:center;color:#788693;font-size:9px;padding:8px}.chat-load-older{min-height:44px;display:block;margin:0 auto 10px;border:0;background:transparent;color:#0b7187;font-size:10px;font-weight:800}
        .chat-empty{min-height:260px;display:grid;place-items:start;align-content:center;gap:8px;max-width:430px;margin:0 auto;text-align:left}.chat-empty strong{font-size:19px}.chat-empty p{margin:0;color:#667684;font-size:12px;line-height:1.55;text-align:left}
        .chat-message{display:grid;grid-template-columns:40px minmax(0,1fr);gap:10px;padding:7px 0}.chat-message.grouped{padding-top:1px}.chat-message.grouped .chat-message-content{grid-column:2}.chat-avatar{width:40px;height:40px;display:grid;place-items:center;border-radius:13px;background:#eaf4f6;color:#0b6074;font-size:10px;font-weight:900}.chat-message.mine .chat-avatar{background:#eaf0f8;color:#214f73}
        .chat-message-content{min-width:0}.chat-message-meta{display:flex;align-items:baseline;gap:8px;margin-bottom:3px}.chat-message-meta strong{font-size:11px}.chat-message-meta span{color:#89949d;font-size:8px}.chat-message-content>p{width:fit-content;max-width:min(620px,92%);margin:0;padding:10px 12px;border-radius:4px 14px 14px 14px;background:#f1f5f7;color:#21313d;font-size:12px;line-height:1.5;white-space:pre-wrap;overflow-wrap:anywhere;text-align:left}.chat-message.mine .chat-message-content>p{background:#eaf3fb}
        .chat-message-actions{display:flex;gap:2px;margin-top:2px}.chat-message-actions button{min-height:44px;border:0;background:transparent;color:#70808d;padding:0 8px;font-size:9px;font-weight:760}.chat-message-actions button:hover{color:#0b7187}

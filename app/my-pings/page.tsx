...
export default function MyPingsPage(){
  ...
  const savedEdit=async()=>{await load("refresh");setMessage("Pin updated. Feed and Map will use the new details.");};
  ...
  return <div className="page-shell"><div className="app-shell"><main className={`${styles.screen} my-pings-v3`}>
    ...
    {!loading&&signedIn===false?<section className={styles.empty}><span><PingIcon name="myPings" size={25}/></span><h2>Your pins live here.</h2><p>Sign in to see and manage the local updates you’ve posted.</p><button type="button" onClick={openAuth}>Sign in / Sign up</button></section>:errorState?<section className={styles.empty}><span><PingIcon name="alert" size={25}/></span><h2>Your pins didn’t load.</h2><p>{loadError}</p><button type="button" onClick={()=>void load("initial")}>Retry</button></section>:<>

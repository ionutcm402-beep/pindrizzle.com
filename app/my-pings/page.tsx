...
function emptyCopy(status:PingStatus){if(status==="active")return"Your live local pins will appear here after you post them.";if(status==="resolved")return"Pins you mark resolved stay here as part of your history.";if(status==="expired")return"Pins that reach their expiry time stay here for you.";return"Removed pins stay in your private history so moderation and audit records remain intact.";}
...
export default function MyPingsPage(){
  ...
  const load=useCallback(async()=>{setLoading(true);setMessage("");try{const supabase=createClient();const{data:authData}=await supabase.auth.getSession();if(!authData.session?.user){setSignedIn(false);setItems([]);return;}setSignedIn(true);const{data,error}=await supabase.rpc("my_pings");if(error)throw error;setItems((data||[])as MyPing[]);}catch(error){console.error("My pins failed",error);setMessage("Your pins could not load right now.");}finally{setLoading(false);}},[]);
  ...
  const resolvePing=async(id:string)=>{setBusyId(id);setMessage("");try{const{error}=await createClient().rpc("resolve_own_ping",{target_ping_id:id});if(error)throw error;setItems((c)=>c.map((i)=>i.id===id?{...i,status:"resolved"}:i));setMenuId(null);setMessage("Pin marked resolved. It is no longer shown as live.");}catch{setMessage("That pin could not be resolved right now.");}finally{setBusyId(null);}};
  const removePing=async(id:string)=>{setBusyId(id);setMessage("");try{const{error}=await createClient().rpc("remove_own_ping",{target_ping_id:id});if(error)throw error;setItems((c)=>c.map((i)=>i.id===id?{...i,status:"removed",has_open_promotion:false}:i));setConfirmRemoveId(null);setMenuId(null);setMessage("Pin removed from community views. Its audit history is preserved.");}catch(error){const text=errorText(error).toLowerCase();setMessage(text.includes("promotion")?"This pin has a promotion in progress. Finish that promotion before removing it.":"That pin could not be removed right now.");}finally{setBusyId(null);}};
  ...
  const savedEdit=async()=>{await load();setMessage("Pin updated. Feed and Map will use the new details.");};

  return <div className="page-shell"><div className="app-shell"><main className={`${styles.screen} my-pings-v3`}>
    <header className="my-pings-v3-header"><div className="brand small">Pindrizzle</div><h1>My Pins</h1><p>What you’ve shared, what is still live, and what has finished.</p></header>
    {!loading&&signedIn===false?<section className={styles.empty}><span><PingIcon name="myPings" size={25}/></span><h2>Your pins live here.</h2><p>Sign in to see and manage the local updates you’ve posted.</p><button type="button" onClick={openAuth}>Sign in / Sign up</button></section>:<>

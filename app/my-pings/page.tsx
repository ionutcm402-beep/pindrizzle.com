...
function emptyCopy(status:PingStatus){if(status==="active")return"Your live local updates will appear here after you post them.";if(status==="resolved")return"Pings you mark resolved stay here as part of your history.";if(status==="expired")return"Pings that reach their expiry time stay here for you.";return"Removed Pings stay in your private history so moderation and audit records remain intact.";}
...
export default function MyPingsPage(){
  ...
  return <div className="page-shell"><div className="app-shell"><main className={`${styles.screen} my-pings-v3`}>
    ...
    {!loading&&signedIn===false?<section className={styles.empty}><span><PingIcon name="myPings" size={25}/></span><h2>Your Pings live here.</h2><p>Sign in to see and manage the local updates you’ve posted.</p><button type="button" onClick={openAuth}>Sign in / Sign up</button></section>:<>
      <div className={`${styles.tabs} my-pings-v3-tabs`} role="tablist" aria-label="My Ping status">{tabs.map((tab)=><button type="button" key={tab.value} role="tab" aria-selected={selected===tab.value} className={selected===tab.value?styles.selectedTab:""} onClick={()=>setSelected(tab.value)}><span>{tab.label}</span><b>{counts[tab.value]}</b></button>)}</div>
      {loading?<section className={styles.empty}><h2>Checking your Pings…</h2></section>:visible.length?<section className={`${styles.list} my-pings-v3-list`}>{visible.map((item)=>{const meta=CATEGORY_DEFINITIONS[item.category]||CATEGORY_DEFINITIONS.local;const menuOpen=menuId===item.id;const confirming=confirmRemoveId===item.id;return <article key={item.id} className={`${styles.card} ${styles[item.status]} my-pings-v3-card`}>

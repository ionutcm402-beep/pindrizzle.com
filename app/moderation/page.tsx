...
export default function ModerationPage() {
  ...
  const load = useCallback(async () => {
    ...
    setAllowed(true);
    const { data, error } = await supabase.rpc("moderation_report_queue");
    if (error) {
      console.error("Moderation queue failed", error);
      setMessage("The moderation queue could not load right now.");
      return;
    }
  ...
  const act = async (row: QueueRow, action: "dismiss" | "remove") => {
    ...
    try {
      const { error } = await createClient().rpc("moderate_report", {
        ...
        moderation_notes: action === "remove" ? "Removed from moderation queue" : "Dismissed from moderation queue",
      });
      if (error) throw error;
      setRows((current) => current.filter((item) => item.report_id !== row.report_id && (action !== "remove" || item.ping_id !== row.ping_id)));
      setMessage(action === "remove" ? "Ping removed. It will no longer appear in Feed or Map." : "Report dismissed. The Ping remains live.");
    } catch (error) {
          ...
          {allowed && (
            <>
              <section className="moderation-intro">
                <div>🛡️</div><div><strong>Review reports carefully.</strong><p>Dismiss weak reports. Remove a Ping only when the content itself should no longer be visible to everyone.</p></div>
              </section>
              {message && <div className="moderation-message">{message}</div>}
              <div className="moderation-count"><strong>{rows.length}</strong><span>{rows.length === 1 ? "pending report" : "pending reports"}</span></div>

              <section className="moderation-list">
                {rows.length ? rows.map((row) => (
                  <article key={row.report_id} className="moderation-card">
                    <div className="moderation-card-top"><span>{reasonLabel(row.reason)}</span><small>{relativeTime(row.reported_at)}</small></div>
                    ...
                    <div className="moderation-signals"><span>{Number(row.reports_on_ping)} {Number(row.reports_on_ping) === 1 ? "report" : "reports"} on this Ping</span><span>Status: {row.ping_status}</span></div>
                ...
                )) : <div className="moderation-empty">No pending reports. ✅</div>}
              </section>
            </>
          )}
        </main>

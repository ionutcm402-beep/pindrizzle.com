...
export default function PromotePage() {
  ...
  const openAuth = () => window.dispatchEvent(new CustomEvent("ping:auth-needed", { detail: { message: "Sign in to promote one of your Pings." } }));
      ...
      <div className="app-shell">
        <main className="phase9-promote-screen">
          <header className="phase9-promote-header">
            ...
            <div><div className="brand small">ping<span>.</span></div><h1>Promote a Ping</h1></div>
          </header>

          <section className="phase9-promote-intro">
            <span>↗</span>
            <div><strong>Reach more people nearby.</strong><p>Promoted Pings stay local, are always labelled as paid placement, and keep the normal Report & Block controls.</p></div>
          </section>

          {!loading && signedIn === false ? (
            <section className="phase9-promote-empty">
              <h2>Sign in to promote a Ping.</h2><p>Create a useful Ping first, then choose how far and how long to promote it.</p><button type="button" onClick={openAuth}>Sign in / Sign up</button>
            </section>
          ) : loading ? (
            ...
            <>
              <section className="phase9-builder">
                <div className="phase9-step-heading"><b>1</b><div><strong>Choose your Ping</strong><span>Only your live, unpromoted Pings are eligible.</span></div></div>
                {pings.length ? (
                  <div className="phase9-ping-options">
                    {pings.map((ping) => {
                      ...
                      return <button key={ping.ping_id} type="button" className={selected ? "selected" : ""} onClick={() => setSelectedPingId(ping.ping_id)}>
                        <div><span>{meta.icon} {meta.label}</span><small>{timeLeft(ping.remaining_minutes)}</small></div><strong>{ping.title}</strong><p>{ping.body}</p>
                      </button>;
                    })}
                  </div>
                ) : <div className="phase9-no-pings"><strong>No eligible Pings right now.</strong><p>Create a new Ping, then come back here while it still has enough time left.</p><a href="/#ping">Create a Ping →</a></div>}
                ...
                <div className="phase9-step-heading"><b>4</b><div><strong>Choose the duration</strong><span>A promotion cannot outlive the original Ping.</span></div></div>
                <div className="phase9-choice-grid duration">
                  {DURATIONS.map((hours) => {
                    const available = durationAvailable(hours);
                    return <button type="button" key={hours} disabled={!available} className={duration === hours ? "selected" : ""} onClick={() => setDuration(hours)}><strong>{hours}h</strong><small>{available ? "Available" : "Ping expires first"}</small></button>;
                  })}
                </div>
              ...
              <section className="phase9-request-history">
                <div className="phase9-history-title"><div><strong>My promotion requests</strong><span>{email}</span></div><b>{requests.length}</b></div>
                {requests.length ? requests.map((request) => <article key={request.promotion_id}>
                  ...
                  <h2>{request.ping_title}</h2><p>From {request.sponsor_name}</p>

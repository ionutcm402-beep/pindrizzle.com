    ...
    <LegalPageShell title="Privacy Notice" kicker="YOUR DATA" active="privacy">
      ...
      <section className="legal-card">
        ...
        <ul>
          ...
          <li><b>Community content:</b> pins, replies, Local Chat messages, photos, confirmations and Helpful signals you choose to submit.</li>
          <li><b>Feed, Map and pin location:</b> device coordinates are used to find nearby activity and create pins. New pins default to Private location, which publishes an approximate nearby area. If you explicitly choose Exact location, the selected exact point is public to people who can see that pin.</li>
          <li><b>Local Chat location:</b> when you use Local Chat, your current location and selected Feed radius are used to decide which nearby group conversation you can read and post to. Chat messages store a snapped approximate area point for radius matching rather than your precise browser coordinate. A short-lived approximate viewer scope is also used so live Realtime updates continue to obey the same radius and block rules.</li>
          ...
          <li><b>Optional product analytics:</b> only after you choose Allow analytics, Pindrizzle records a random browser-session identifier and coarse events such as Feed, Map, Search or Local Chat use. It does not record chat message text, search text or exact location in the product analytics table.</li>
        </ul>
      </section>

      <section className="legal-card">
        <h2>Why the information is used</h2>
        <p>Pindrizzle uses account and service data to provide the features you ask for, including authentication, nearby results, pins, Local Chat, replies, notifications and promotions when enabled. Safety and security information is used to prevent abuse, investigate reports and protect the service. Optional product analytics is based on your browser choice and is used to understand whether core product areas are useful.</p>
      ...
      <section className="legal-card">
        ...
        <p><b>Local Chat is not private messaging.</b> A Local Chat message is visible to other signed-in users whose current Local Chat scope overlaps the relevant selected radius and who are not excluded by block/report controls. Pindrizzle does not display the stored approximate chat point in the Chat interface, but the approximate point is part of the message record used to enforce radius visibility.</p>
      ...
      <section className="legal-card">
        <h2>Retention</h2>
        <p>Active pins normally stop being publicly available after their chosen category-specific expiry, but expiry is not the same as immediate database deletion.</p>
        <p><b>Local Chat messages use a 90-day default retention period.</b> A daily automated cleanup anonymises chat messages older than 90 days by removing the author link and stored location and replacing the message body with an expiry marker. A message with a still-pending safety/moderation report can be held past 90 days until that human review is resolved; once the hold no longer applies, the next cleanup can anonymise it.</p>
        <p>Safety, moderation, payment, security and legal records may need to be retained longer than the underlying community content. Raw optional product analytics is designed to age out after up to 90 days. Account data is retained while the account exists unless deletion is appropriate, subject to safety, fraud, payment or legal retention needs. The Local Chat retention period and any formal evidence-preservation exceptions should be confirmed in the final legal review before wide launch.</p>
      </section>

      <section className="legal-card">
        <h2>Service providers</h2>
        <p>Pindrizzle currently relies on Supabase for database/authentication/storage and Realtime services, Vercel for website hosting, browser/web-push infrastructure for push delivery, and OpenStreetMap-derived map/place services for map and place features. Stripe is used for payment processing only when paid features are enabled. Those providers can receive ordinary technical request information needed to deliver their services.</p>

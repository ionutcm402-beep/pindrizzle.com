...
export default function MapPage(){
  ...
  useEffect(()=>{
    ...
    let cancelled=false;
    void getPingLocationSilently().then((result)=>{if(cancelled)return;setLocationState(result.state);if(result.coordinates)setCenter(result.coordinates);else setStatus(result.state==="denied"?"Location is off.":"Turn on location to use Feed and Map.");});
  ...
  {filterOpen&&<div className="map-v3-filter-backdrop" role="dialog" aria-modal="true" aria-label="Map filters" onClick={()=>setFilterOpen(false)}><section className="map-v3-filter-sheet" onClick={(event)=>event.stopPropagation()}><div className="sheet-handle"/><div className="map-v3-filter-head"><div><span>FEED + MAP FILTER</span><h2>Choose what to show</h2></div><button type="button" onClick={()=>setFilterOpen(false)}>Done</button></div><div className="map-v3-filter-grid"><button type="button" className={filter==="all"?"active":""} onClick={()=>{writePingCategory("all");setFilterOpen(false);}}>All categories</button>{CATEGORY_ORDER.map((key)=>{const item=CATEGORY_DEFINITIONS[key];return<button type="button" key={key} className={filter===key?"active":""} onClick={()=>{writePingCategory(key);if(key!=="marketplace")setFilterOpen(false);}}><PingIcon name={item.icon} size={16}/>{item.label}</button>;})}</div>{filter==="marketplace"&&<section className="map-v3-market-filters"><div><strong>Marketplace</strong><span>Changes also apply to Feed.</span></div><label>Type<select value={marketplaceTypeFilter} onChange={(event)=>writeMarketplaceType(event.target.value as MarketplaceTypeFilter)}><option value="all">Everything</option>{MARKETPLACE_TYPES.map((value)=><option key={value} value={value}>{MARKETPLACE_TYPE_LABEL[value]}</option>)}</select></label><label>Looking for<select value={marketplaceIntentFilter} onChange={(event)=>writeMarketplaceIntent(event.target.value as MarketplaceIntentFilter)}><option value="all">Sale, rent or wanted</option>{MARKETPLACE_INTENTS.map((value)=><option key={value} value={value}>{MARKETPLACE_INTENT_LABEL[value]}</option>)}</select></label><label>Maximum price<select value={marketplaceMaxPrice??""} onChange={(event)=>writeMarketplaceMaxPrice(event.target.value?Number(event.target.value):null)}><option value="">Any price</option>{MARKETPLACE_PRICE_FILTERS.map((price)=><option key={price} value={price}>Up to {formatMarketplacePrice(price,"total")}</option>)}</select></label></section>}</section></div>}

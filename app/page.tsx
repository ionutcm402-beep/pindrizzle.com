...
function Composer({ onClose, onPublish }: { onClose: () => void; onPublish: (draft: PingDraft) => void | Promise<void> }) {
  ...
  return <div className="composer-backdrop" role="dialog" aria-modal="true" aria-label="Create a Ping"><div className="composer-sheet composer-v3-sheet">
    <div className="sheet-handle" /><div className="composer-header"><button onClick={onClose} disabled={publishing}>Cancel</button><strong>New Ping</strong><span /></div><h2>What should people nearby know?</h2>
    <div className="composer-v3-category-grid" aria-label="Ping category">{CATEGORY_ORDER.map((key) => { const item = CATEGORY_DEFINITIONS[key]; return <button type="button" key={key} className={category === key ? "selected" : ""} onClick={() => chooseCategory(key)} disabled={publishing}><PingIcon name={item.icon} size={16} /><span>{item.label}</span></button>; })}</div>

    {category === "deals" && <section className="composer-v3-deal-panel"><div className="composer-v3-source-toggle"><button type="button" className={dealSource === "spotted" ? "selected" : ""} onClick={() => setDealSource("spotted")}><PingIcon name="deals" size={15} />I found this deal</button><button type="button" className={dealSource === "business" ? "selected" : ""} onClick={() => setDealSource("business")}><PingIcon name="business" size={15} />Business post</button></div><label>Shop or business name<input value={merchantName} onChange={(event) => setMerchantName(event.target.value)} maxLength={120} placeholder="e.g. Tesco, local café, Currys" /></label><label>What kind of deal?<select value={dealKind} onChange={(event) => setDealKind(event.target.value as DealKind)}>{DEAL_KINDS.map((kind) => <option key={kind} value={kind}>{DEAL_KIND_LABEL[kind]}</option>)}</select></label>{dealSource === "business" && <small>Business posts are self-identified for now. Ping will not call them verified until a verification system exists.</small>}</section>}
    ...
    {category === "marketplace" && <section className="composer-v3-market-panel">
      ...
      <small>Use the link when the full listing already lives on another website. Ping stays the local discovery layer.</small>
    </section>}

    <label className="composer-label">Short headline</label><input className="composer-input" placeholder={category === "deals" ? "e.g. Air fryer reduced to £39" : category === "marketplace" ? marketplacePlaceholder : "What should neighbours know?"} maxLength={70} value={title} onChange={(event) => setTitle(event.target.value)} disabled={publishing} />
    <label className="composer-label">Useful detail</label><textarea placeholder={category === "deals" ? "Price, stock, where you saw it and anything people should know…" : category === "marketplace" ? "Key details people nearby need before they open the full listing…" : "Keep it clear and useful…"} maxLength={280} value={body} onChange={(event) => setBody(event.target.value)} disabled={publishing} />
    <div className="composer-v3-expiry"><label><span>Keep this Ping live for</span><select value={expiryHours} onChange={(event) => setExpiryHours(Number(event.target.value))}>{expiryOptionsForCategory(category).map((hours) => <option key={hours} value={hours}>{hours < 24 ? `${hours} hours` : `${hours / 24} ${hours === 24 ? "day" : "days"}`}</option>)}</select></label><small>{category === "marketplace" ? "Marketplace listings can stay discoverable for up to 30 days. Resolve them earlier when sold, rented or no longer wanted." : "Ping recommends shorter lifetimes for traffic, parking and alerts, and longer ones for Lost & Found."}</small></div>
    ...
    {photoPreview && <div className="composer-photo-preview"><img src={photoPreview} alt="Selected Ping photo preview" /><button type="button" onClick={() => choosePhoto(null)} disabled={publishing}>Remove</button></div>}{photoError && <div className="composer-photo-error">{photoError}</div>}
    <div className="expiry-note">Your browser location is snapped to an approximate public area before publishing.</div><button className="publish-button" disabled={!canPublish} onClick={publish}>{publishing ? "Posting…" : "Ping it"}</button>
  </div></div>;
}
...
export default function Home() {
  ...
  const beginCompose = () => { setPendingCompose(true); if (!authReady) return; if (!userId) { requestAuth("Sign in once to post useful Pings nearby."); return; } if (locationState !== "granted" || !coordinates) void requestLocation(); };
  ...
  const confirmPing = async (id: string) => { if (!userId) { requestAuth("Sign in to confirm useful Pings near you."); return; } try { const { data, error } = await createClient().rpc("confirm_ping", { target_ping_id: id }); if (error) throw error; const now = new Date().toISOString(); setPings((current) => current.map((ping) => ping.id === id ? { ...ping, confirmations: Number(data), lastConfirmedAt: now } : ping)); } catch {} };
  ...
  const publishPing = async (draft: PingDraft) => {
    ...
    try {
      ...
      if (draft.photo && createdId) { const storagePath = `${userId}/${createdId}/photo`; const upload = await supabase.storage.from("ping-media").upload(storagePath, draft.photo, { cacheControl: "3600", contentType: draft.photo.type, upsert: false }); if (upload.error) { console.error("Ping photo upload failed", upload.error); window.alert("Your Ping was posted, but the photo could not upload. The text Ping is still live."); } else { const attach = await supabase.rpc("attach_ping_media", { target_ping_id: createdId, object_path: storagePath, media_mime_type: draft.photo.type, media_byte_size: draft.photo.size }); if (attach.error) { console.error("Ping photo attach failed", attach.error); await supabase.storage.from("ping-media").remove([storagePath]); window.alert("Your Ping was posted, but the photo could not be attached. The text Ping is still live."); } } }
      setComposerOpen(false); setRefreshNonce((value) => value + 1);
    } catch (error) { console.error("Publish failed", error); const message = error && typeof error === "object" && "message" in error ? String((error as { message?: unknown }).message || "") : ""; window.alert(message || "Ping couldn’t publish yet. Please try again."); }
  };
  ...
  const summary = visible.length ? visible.slice(0, 2).map((ping) => ping.category === "marketplace" && ping.marketplacePrice != null ? `${formatMarketplacePrice(ping.marketplacePrice, ping.marketplacePricePeriod, ping.marketplaceCurrency || "GBP", true)} · ${ping.distanceMiles.toFixed(1)} mi` : `${CATEGORY_DEFINITIONS[ping.category].shortLabel} ${ping.distanceMiles.toFixed(1)} mi`).join(" · ") : dataMode === "offline" ? "Live local data is temporarily unavailable." : "Real updates near you, ordered by usefulness.";
  ...
  return <div className="page-shell"><div className="app-shell"><div className="screen-content">
    <header className="app-header feed-v3-header"><div><div className="brand">ping<span>.</span></div><div className="location-pill">{locationState === "granted" ? "● Your local area" : "○ Location off"}</div></div></header>
    ...
    <main className="feed-list feed-v3-list">{visible.length ? visible.map((ping) => <FeedCard key={ping.id} ping={ping} onConfirm={confirmPing} onOpen={openPingDetail} />) : <div className="quiet-card feed-v3-quiet"><div className="quiet-icon"><PingIcon name={locationState === "granted" ? "check" : "location"} size={25} /></div><h2>{locationState !== "granted" ? "Your local feed starts with location." : dataMode === "offline" ? "We couldn’t load nearby Pings." : `Quiet within ${radius} ${radius === 1 ? "mile" : "miles"}.`}</h2><p>{locationState !== "granted" ? "Enable location once and Ping will reuse that permission across Feed and Map." : dataMode === "offline" ? "Try again shortly." : wider ? `${wider.items.length} useful ${wider.items.length === 1 ? "Ping" : "Pings"} exist within ${wider.radius} miles.` : filter === "marketplace" ? "No nearby listings match these Marketplace filters right now." : "Nothing active has been reported in this category nearby right now."}</p>{wider && <button type="button" className="feed-v3-widen" onClick={() => writePingRadius(wider.radius)}>See {wider.radius} mi</button>}</div>}</main>
  </div></div>

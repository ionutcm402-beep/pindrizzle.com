create table if not exists public.retention_visits (
...
alter table public.retention_visits enable row level security;
revoke all on table public.retention_visits from anon, authenticated;

create or replace function public.retention_since_last_visit(
...
begin
  ...
  if prior_visit is null then
    insert into public.retention_visits (user_id, last_visit_at, created_at, updated_at)
    values (auth.uid(), now(), now(), now())
    on conflict (user_id) do update set last_visit_at = excluded.last_visit_at, updated_at = now();
  ...
  update public.retention_visits
...
revoke all on function public.retention_since_last_visit(double precision, double precision, integer) from public, anon;
grant execute on function public.retention_since_last_visit(double precision, double precision, integer) to authenticated;

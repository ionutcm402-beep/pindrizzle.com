...
begin
  if not exists (select 1 from pg_constraint where conname = 'pings_deal_source_check') then
    alter table public.pings add constraint pings_deal_source_check check (deal_source is null or deal_source in ('spotted','business'));
  end if;
  if not exists (select 1 from pg_constraint where conname = 'pings_deal_kind_check') then
    alter table public.pings add constraint pings_deal_kind_check check (deal_kind is null or deal_kind in ('offer','new_in','restock','clearance','limited_time'));
  end if;
  if not exists (select 1 from pg_constraint where conname = 'pings_merchant_name_check') then
    alter table public.pings add constraint pings_merchant_name_check check (merchant_name is null or char_length(trim(merchant_name)) between 2 and 120);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'pings_deal_metadata_scope_check') then
    alter table public.pings add constraint pings_deal_metadata_scope_check check (
...
create index if not exists confirmations_ping_created_idx on public.confirmations (ping_id, created_at desc);

update public.pings p
set last_confirmed_at = x.last_confirmed_at
from (select ping_id, max(created_at) as last_confirmed_at from public.confirmations group by ping_id) x
where p.id = x.ping_id and p.last_confirmed_at is distinct from x.last_confirmed_at;
...
begin
  if tg_op = 'DELETE' then target_id := old.ping_id; else target_id := new.ping_id; end if;
  update public.pings
  set last_confirmed_at = (select max(c.created_at) from public.confirmations c where c.ping_id = target_id)
  where id = target_id;
  return null;
end;
$$;
...
create trigger sync_ping_last_confirmed_at after insert or delete on public.confirmations for each row execute function public.sync_ping_last_confirmed_at();
...
begin
  ...
  if ping_lat is null or ping_lng is null or ping_lat < -90 or ping_lat > 90 or ping_lng < -180 or ping_lng > 180 then
  ...
  insert into public.pings (user_id, category, title, body, location, location_precision, place_label, expires_at, deal_source, deal_kind, merchant_name)
  values (auth.uid(), ping_category, ping_title, ping_body, approximate_location, 'approximate', resolved_place, now() + make_interval(hours => effective_hours), clean_source, clean_kind, clean_merchant)

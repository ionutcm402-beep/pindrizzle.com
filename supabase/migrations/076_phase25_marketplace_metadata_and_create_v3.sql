-- Phase 25.5: structured Marketplace listings and create_ping_v3.
-- Marketplace intentionally stays one top-level category; property, vehicle and
-- parking-space listing details are metadata used by Feed/Map filters.

alter table public.pings
  add column if not exists marketplace_type text,
  add column if not exists marketplace_intent text,
  add column if not exists marketplace_subtype text,
  add column if not exists marketplace_price numeric(12,2),
  add column if not exists marketplace_price_period text,
  add column if not exists marketplace_currency text,
  add column if not exists marketplace_url text;

do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'pings_marketplace_type_check') then
    alter table public.pings add constraint pings_marketplace_type_check check (marketplace_type is null or marketplace_type in ('property','vehicle','parking_space'));
  end if;
  if not exists (select 1 from pg_constraint where conname = 'pings_marketplace_intent_check') then
    alter table public.pings add constraint pings_marketplace_intent_check check (marketplace_intent is null or marketplace_intent in ('sale','rent','wanted'));
  end if;
  if not exists (select 1 from pg_constraint where conname = 'pings_marketplace_subtype_check') then
    alter table public.pings add constraint pings_marketplace_subtype_check check (
      marketplace_subtype is null or
      (marketplace_type = 'property' and marketplace_subtype in ('house','flat','room','land','warehouse','commercial','garage','other')) or
  ...
  if not exists (select 1 from pg_constraint where conname = 'pings_marketplace_price_check') then
    alter table public.pings add constraint pings_marketplace_price_check check (marketplace_price is null or (marketplace_price >= 0 and marketplace_price <= 999999999.99));
  end if;
  if not exists (select 1 from pg_constraint where conname = 'pings_marketplace_price_period_check') then
    alter table public.pings add constraint pings_marketplace_price_period_check check (marketplace_price_period is null or marketplace_price_period in ('total','month','week','day','hour'));
  end if;
  if not exists (select 1 from pg_constraint where conname = 'pings_marketplace_currency_check') then
    alter table public.pings add constraint pings_marketplace_currency_check check (marketplace_currency is null or marketplace_currency ~ '^[A-Z]{3}$');
  end if;
  if not exists (select 1 from pg_constraint where conname = 'pings_marketplace_url_check') then
    alter table public.pings add constraint pings_marketplace_url_check check (marketplace_url is null or (char_length(marketplace_url) <= 500 and marketplace_url ~* '^https?://'));
  end if;
  if not exists (select 1 from pg_constraint where conname = 'pings_marketplace_price_shape_check') then
    alter table public.pings add constraint pings_marketplace_price_shape_check check (
      (marketplace_price is null and marketplace_price_period is null and marketplace_currency is null)
      or
      (marketplace_price is not null and marketplace_price_period is not null and marketplace_currency is not null)
    );
  end if;
  if not exists (select 1 from pg_constraint where conname = 'pings_marketplace_metadata_scope_check') then
    alter table public.pings add constraint pings_marketplace_metadata_scope_check check (
      (category = 'marketplace'::public.ping_category and marketplace_type is not null and marketplace_intent is not null and marketplace_subtype is not null)
      or
      (category <> 'marketplace'::public.ping_category and marketplace_type is null and marketplace_intent is null and marketplace_subtype is null and marketplace_price is null and marketplace_price_period is null and marketplace_currency is null and marketplace_url is null)
    );
  end if;
end $$;
...
begin
  ...
  if ping_category = 'marketplace'::public.ping_category then
    ...
    if clean_market_type not in ('property','vehicle','parking_space') then raise exception 'Choose Property, Vehicle or Parking space'; end if;
    if clean_market_intent not in ('sale','rent','wanted') then raise exception 'Choose For sale, For rent or Wanted'; end if;
    if clean_market_type = 'property' and clean_market_subtype not in ('house','flat','room','land','warehouse','commercial','garage','other') then raise exception 'Invalid property type'; end if;

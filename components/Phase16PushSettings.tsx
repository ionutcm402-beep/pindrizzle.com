...
import { disableNativePush, enableNativePush, readNativePushState, supportsNativePush } from "@/lib/native-push";

type PushState = "checking" | "unsupported" | "off" | "blocked" | "on" | "working";
type Props = { userId: string | null; authLoading: boolean };
...
export default function Phase16PushSettings({ userId, authLoading }: Props) {
  ...
  const webSupported = useCallback(() => typeof window !== "undefined" && window.isSecureContext && "serviceWorker" in navigator && "PushManager" in window && "Notification" in window, []);

  const refresh = useCallback(async () => {
    if (supportsNativePush()) {
      try {
        const nativeState = await readNativePushState(userId);
        const stateResult = userId ? await createClient().rpc("my_push_state") : { data: null };
    ...
    if (!userId) { setState("off"); setDeviceCount(0); return; }
  ...
  const enable = async () => {
    ...
    if (supportsNativePush()) {
      try {
        ...
        const text = error instanceof Error ? error.message : "Notifications could not be enabled on this device yet.";
        setState(text.toLowerCase().includes("permission") ? "blocked" : "off");
        setMessage(text);
      }
      return;
    }
    ...
    try {
      ...
      if (permission === "denied") { setState("blocked"); setMessage("Notifications are blocked in this browser’s site settings."); return; }
      ...
      if (!subscription) subscription = await registration.pushManager.subscribe({ userVisibleOnly: true, applicationServerKey: vapidKeyToBytes(publicKey) });
      const keys = subscriptionKeys(subscription);
  ...
  const disable = async () => {
    ...
    if (supportsNativePush()) {
      try {
        ...
        setState("off"); setMessage("Pindrizzle notifications are off on this device.");
        await refresh();
      } catch (error) {
        console.error("Disable native push failed", error);
    ...
    try {
      ...
      setState("off"); setMessage("Pindrizzle notifications are off on this device."); await refresh();
    } catch (error) {
      console.error("Disable push failed", error);
      setMessage("Notification settings could not be changed right now."); await refresh();
    }
  };

  const statusCopy = state === "on" ? "Enabled on this device" : state === "blocked" ? "Blocked by device settings" : state === "unsupported" ? "Not available on this device" : state === "working" || state === "checking" ? "Checking this device…" : "Off on this device";

  return <section className="phase16-push-card" aria-label="Pindrizzle notification settings">
    <div className="phase16-push-icon"><PingIcon name="alerts" size={20} /></div>
    <div className="phase16-push-copy"><div className="phase16-push-title"><strong>Push notifications</strong><span className={state === "on" ? "on" : ""}>{statusCopy}</span></div><p>Get useful local activity even when Pindrizzle is closed. Notifications follow your Reply, Confirmation and Helpful choices below.</p>{deviceCount > 0 && <small>{deviceCount} {deviceCount === 1 ? "device" : "devices"} connected to this account.</small>}{message && <div className="phase16-push-message">{message}</div>}</div>
    <button type="button" onClick={state === "on" ? disable : enable} disabled={authLoading || state === "checking" || state === "working" || state === "blocked" || state === "unsupported"}>{state === "on" ? "Turn off" : state === "working" ? "Working…" : "Enable push"}</button>
    <style jsx>{`

...
export default function MapHome(){
  ...
  const beginCompose=useCallback(()=>{setPendingCompose(true);if(!authReady)return;if(!userId){requestAuth("Sign in to publish a pin.");return;}if(locationState!=="granted"||!center)void requestLocation();},[authReady,userId,locationState,center,requestLocation]);
  useEffect(()=>{if(!authReady||window.location.hash!=="#ping")return;window.history.replaceState({},"","/");beginCompose();},[authReady,beginCompose]);
  useEffect(()=>{if(!pendingCompose||!userId)return;if(locationState==="idle"||locationState==="unavailable"){void requestLocation();return;}if(locationState==="granted"&&center){setComposerOpen(true);setPendingCompose(false);}},[pendingCompose,userId,locationState,center,requestLocation]);

  const publishPing=useCallback(async(draft:PingDraft)=>{
    if(!userId){setComposerOpen(false);requestAuth("Sign in to publish a pin.");return;}
    if(!center){setComposerOpen(false);setPendingCompose(true);void requestLocation();return;}
    try{
      ...
      const{data,error}=await supabase.rpc("create_ping_v3",{
        ping_category:draft.category,ping_title:draft.title,ping_body:draft.body,ping_lat:center.lat,ping_lng:center.lng,ping_place_label:"Near your current location",ping_expires_in_hours:draft.expiryHours,
      ...
      setComposerOpen(false);setRefreshKey((value)=>value+1);
  ...
  const pings=useMemo(()=>{const base=filter==="all"?allPings:allPings.filter((ping)=>ping.categoryKey===filter);if(filter!=="marketplace")return base;return base.filter((ping)=>marketplaceListingMatches(marketplaceListingTypeFilter,ping.marketplaceType,ping.marketplaceIntent,ping.marketplaceSubtype)&&(marketplaceMaxPrice==null||(ping.marketplacePrice!=null&&ping.marketplacePrice<=marketplaceMaxPrice)));},[allPings,filter,marketplaceListingTypeFilter,marketplaceMaxPrice]);
  ...
  const selectedMarketplaceListing=selected?.marketplaceListingType?MARKETPLACE_LISTING_DEFINITIONS[selected.marketplaceListingType]:null;
  ...
  const chooseMarketplaceListingFilter=useCallback((value:MarketplaceListingTypeFilter)=>{writePingCategory("marketplace");writeMarketplaceListingType(value);},[]);
  ...
  return <div className="page-shell map-v3-page-shell"><div className="app-shell map-v3-app-shell"><main className="map-v3-screen launch-map-screen">
    ...
    {selected&&selectedDefinition&&<section className="map-v3-card" aria-label="Selected pin"><button type="button" className="map-v3-card-main" onClick={openSelected}><div className="map-v3-card-top"><span><i><PingIcon name={selectedDefinition.icon} size={15}/></i>{selectedDefinition.label}</span><b>{selected.distanceMiles.toFixed(1)} mi away</b></div>{selected.categoryKey==="deals"&&selected.merchantName&&<div className="map-v3-merchant"><PingIcon name={selected.dealSource==="business"?"business":"deals"} size={13}/><strong>{selected.merchantName}</strong>{selected.dealSource&&<span>{DEAL_SOURCE_LABEL[selected.dealSource]}</span>}{selected.dealKind&&<span>· {DEAL_KIND_LABEL[selected.dealKind]}</span>}</div>}{selected.categoryKey==="marketplace"&&selected.marketplaceType&&selected.marketplaceIntent&&<div className="map-v3-market"><div><PingIcon name={selectedMarketplaceListing?.icon||(selected.marketplaceType==="property"?"property":selected.marketplaceType==="vehicle"?"vehicle":"parking")} size={13}/><strong>{selectedMarketplaceListing?.label||marketplaceSubtypeLabel(selected.marketplaceType,selected.marketplaceSubtype)}</strong>{!selectedMarketplaceListing&&<span>{MARKETPLACE_INTENT_LABEL[selected.marketplaceIntent]}</span>}</div><b>{formatMarketplacePrice(selected.marketplacePrice,selected.marketplacePricePeriod,selected.marketplaceCurrency||"GBP")}</b></div>}<h2>{selected.title}</h2><footer><span><PingIcon name="confirmations" size={14}/>{selected.confirmations} confirmed{selected.lastConfirmedAt?` · ${freshness(selected.lastConfirmedAt)}`:""}</span><strong>Open →</strong></footer></button>{pings.length>1&&<div className="map-v3-card-pager"><button type="button" onClick={()=>stepSelected(-1)} aria-label="Previous nearby pin">‹</button><span>{selectedIndex+1} of {pings.length}</span><button type="button" onClick={()=>stepSelected(1)} aria-label="Next nearby pin">›</button></div>}</section>}
  ...
  <style jsx global>{`
    ...
    .map-v3-category-chips .map-v3-market-chip{border-color:rgba(8,43,73,.13);background:rgba(255,255,255,.92)}.map-v3-category-chips .map-v3-market-chip-for_sale.active{border-color:#082b49;background:#082b49;color:#fff}.map-v3-category-chips .map-v3-market-chip-to_rent.active{border-color:#317fc1;background:#317fc1;color:#fff}.map-v3-category-chips .map-v3-market-chip-car.active{border-color:#a04d78;background:#a04d78;color:#fff}.map-v3-category-chips .map-v3-market-chip-parking_space.active{border-color:#596b85;background:#596b85;color:#fff}
    ...
    .map-v3-filter-backdrop{position:fixed;inset:0;z-index:180;background:rgba(11,15,12,.28);display:flex;align-items:flex-end;justify-content:center}.map-v3-filter-sheet{width:min(100%,480px);max-height:82dvh;overflow:auto;border-radius:24px 24px 0 0;background:var(--ping-canvas);padding:10px 18px max(24px,env(safe-area-inset-bottom));box-shadow:0 -18px 50px rgba(16,19,17,.18)}.map-v3-filter-head{display:flex;align-items:end;justify-content:space-between;gap:12px;margin:8px 0 14px}.map-v3-filter-head span{font-size:8px;font-weight:800;letter-spacing:.1em;color:var(--ping-muted-2)}.map-v3-filter-head h2{margin:4px 0 0;font-size:20px;letter-spacing:-.5px}.map-v3-filter-head button{border:0;background:transparent;color:var(--ping-accent-ink);font-size:10px;font-weight:800}.map-v3-filter-section-title{margin:14px 0 8px;color:var(--ping-muted);font-size:9px;text-transform:uppercase;letter-spacing:.08em}.map-v3-filter-section-title.places{margin-top:18px}.map-v3-filter-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.map-v3-filter-grid button{min-height:44px;display:flex;align-items:center;justify-content:flex-start;gap:8px;border:1px solid var(--ping-line);border-radius:12px;background:#fff;color:var(--ping-ink-2);padding:0 11px;font-size:9px;font-weight:720}.map-v3-filter-grid button.active{border-color:var(--ping-ink);background:var(--ping-ink);color:#fff}.map-v3-market-filters{margin-top:14px;padding:12px;border:1px solid var(--ping-line);border-radius:14px;background:#fff;display:grid;grid-template-columns:1fr;gap:8px}.map-v3-market-filters>div strong,.map-v3-market-filters>div span{display:block}.map-v3-market-filters>div strong{font-size:11px}.map-v3-market-filters>div span{margin-top:2px;color:var(--ping-muted);font-size:8px}.map-v3-market-type-grid{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px}.map-v3-market-type-grid button{min-height:38px;display:flex;align-items:center;justify-content:flex-start;gap:6px;border:1px solid var(--ping-line);border-radius:10px;background:var(--ping-surface-soft);color:var(--ping-ink-2);padding:0 9px;font-size:8px;font-weight:740}.map-v3-market-type-grid button.active{border-color:var(--ping-ink);background:var(--ping-ink);color:#fff}.map-v3-market-filters label{display:grid;gap:5px;color:var(--ping-muted);font-size:8px;font-weight:750}.map-v3-market-filters select{height:38px;min-width:0;border:1px solid var(--ping-line);border-radius:10px;background:var(--ping-surface-soft);padding:0 8px;color:var(--ping-ink-2);font-size:9px}.map-v3-place-filter-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.map-v3-place-filter-grid button{min-height:46px;display:grid;grid-template-columns:12px 1fr auto;gap:7px;align-items:center;border:1px solid var(--ping-line);border-radius:12px;background:#fff;color:var(--ping-ink-2);padding:0 10px;text-align:left;font-size:9px;font-weight:740}.map-v3-place-filter-grid button>span{width:10px;height:10px;border-radius:3px;background:#27865e}.map-v3-place-filter-grid button.place-toilets>span{background:#6b5fc7}.map-v3-place-filter-grid button.place-restaurant>span{background:#df715b}.map-v3-place-filter-grid button.place-playground>span{background:#d59a2d}.map-v3-place-filter-grid button:not(.active){opacity:.52}.map-v3-place-filter-grid button b{color:var(--ping-muted);font-size:7px}.map-v3-osm-note{margin:9px 0 0;color:var(--ping-muted-2);font-size:8px;line-height:1.45}
    .map-v3-drop-pin{position:absolute;z-index:9;right:14px;bottom:18px;height:46px;display:flex;align-items:center;gap:7px;border:0;border-radius:999px;padding:0 18px;background:var(--ping-ink);color:#fff;font-size:12px;font-weight:780;box-shadow:0 12px 28px rgba(16,19,17,.22)}.map-v3-drop-pin:active{transform:scale(.97)}
    @keyframes mapV3Pulse{50%{opacity:.3}}

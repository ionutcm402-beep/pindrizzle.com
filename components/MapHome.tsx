...
export default function MapHome(){
  ...
  return <div className="page-shell"><div className="app-shell"><main className="map-v3-screen launch-map-screen">
    ...
    {center&&<section className="map-v3-controls" aria-label="Map controls"><div className="map-v3-status"><span className={dataBusy?"busy":""}/><strong>{displayStatus}</strong></div><div className="map-v3-control-row"><div className="map-v3-radii" aria-label="Nearby radius">{RADII.map((option)=><button type="button" key={option} className={radius===option?"active":""} onClick={()=>{writePingRadius(option);setSelectedId(null);}} aria-label={`${option} ${option===1?"mile":"miles"}`}>{option} mi</button>)}</div><button type="button" className="map-v3-filter-button" onClick={()=>setFilterOpen(true)} aria-label="Open detailed filters"><span>Filters</span><PingIcon name="more" size={17}/></button></div><div className="map-v3-category-chips" aria-label="Pin categories"><button type="button" className={filter==="all"?"active":""} onClick={()=>writePingCategory("all")}>All</button>{CATEGORY_ORDER.map((key)=>{const item=CATEGORY_DEFINITIONS[key];return <button type="button" key={key} className={filter===key?"active":""} onClick={()=>{writePingCategory(key);if(key==="marketplace"&&filter==="marketplace")setFilterOpen(true);}}><PingIcon name={item.icon} size={12}/>{item.shortLabel}</button>;})}</div></section>}
  ...
  {filterOpen&&<div className="map-v3-filter-backdrop" role="dialog" aria-modal="true" aria-label="Map filters" onClick={()=>setFilterOpen(false)}><section className="map-v3-filter-sheet" onClick={(event)=>event.stopPropagation()}><div className="sheet-handle"/><div className="map-v3-filter-head"><div><span>MAP FILTERS</span><h2>Choose what to show</h2></div><button type="button" onClick={()=>setFilterOpen(false)}>Done</button></div><div className="map-v3-filter-grid"><button type="button" className={filter==="all"?"active":""} onClick={()=>{writePingCategory("all");setFilterOpen(false);}}>All categories</button>{CATEGORY_ORDER.map((key)=>{const item=CATEGORY_DEFINITIONS[key];return<button type="button" key={key} className={filter===key?"active":""} onClick={()=>{writePingCategory(key);if(key!=="marketplace")setFilterOpen(false);}}><PingIcon name={item.icon} size={16}/>{item.label}</button>;})}</div>{filter==="marketplace"&&<section className="map-v3-market-filters"><div><strong>Marketplace</strong><span>These preferences are shared with the Feed.</span></div><label>Type<select value={marketplaceTypeFilter} onChange={(event)=>writeMarketplaceType(event.target.value as MarketplaceTypeFilter)}><option value="all">Everything</option>{MARKETPLACE_TYPES.map((value)=><option key={value} value={value}>{MARKETPLACE_TYPE_LABEL[value]}</option>)}</select></label><label>Looking for<select value={marketplaceIntentFilter} onChange={(event)=>writeMarketplaceIntent(event.target.value as MarketplaceIntentFilter)}><option value="all">Sale, rent or wanted</option>{MARKETPLACE_INTENTS.map((value)=><option key={value} value={value}>{MARKETPLACE_INTENT_LABEL[value]}</option>)}</select></label><label>Maximum price<select value={marketplaceMaxPrice??""} onChange={(event)=>writeMarketplaceMaxPrice(event.target.value?Number(event.target.value):null)}><option value="">Any price</option>{MARKETPLACE_PRICE_FILTERS.map((price)=><option key={price} value={price}>Up to {formatMarketplacePrice(price,"total")}</option>)}</select></label></section>}</section></div>}

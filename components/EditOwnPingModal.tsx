...
export default function EditOwnPingModal({ pingId, onClose, onSaved }: Props) {
  ...
  const priceValid = parsedPrice == null || (Number.isFinite(parsedPrice) && parsedPrice >= 0 && parsedPrice <= 999999999.99);
  const dealValid = ping?.category !== "deals" || merchantName.trim().length >= 2;
  ...
  const valid = textValid && dealValid && !exactNeedsConfirmation;
  const category = ping?.category;
  const definition = category ? CATEGORY_DEFINITIONS[category] : null;
  const marketplaceListingType = marketplaceListingTypeFromMetadata(marketplaceType, marketplaceIntent, marketplaceSubtype);
  const marketplaceListingDefinition = marketplaceListingType ? MARKETPLACE_LISTING_DEFINITIONS[marketplaceListingType] : null;
  const expiryOptions = useMemo(() => category ? expiryOptionsForCategory(category) : [], [category]);

  const chooseMarketplaceListingType = (next: MarketplaceListingType) => {
    const item = MARKETPLACE_LISTING_DEFINITIONS[next];
    setMarketplaceType(item.marketplaceType);
    setMarketplaceIntent(item.marketplaceIntent);
    setMarketplaceSubtype(item.marketplaceSubtype);
    setMarketplacePricePeriod(item.pricePeriod);
  };
  ...
  return <div className="edit-ping-backdrop" role="dialog" aria-modal="true" aria-label="Edit Ping"><section className="edit-ping-sheet">
    ...
    {loading ? <div className="edit-ping-loading">Loading your Ping…</div> : ping && definition ? <>
      ...
      {ping.category === "deals" && <section className="edit-ping-special"><div className="edit-ping-toggle"><button type="button" className={dealSource === "spotted" ? "selected" : ""} onClick={() => setDealSource("spotted")}>{DEAL_SOURCE_LABEL.spotted}</button><button type="button" className={dealSource === "business" ? "selected" : ""} onClick={() => setDealSource("business")}>{DEAL_SOURCE_LABEL.business}</button></div><label>Shop or business name<input value={merchantName} onChange={e => setMerchantName(e.target.value)} maxLength={120}/></label>{!dealValid && <small className="edit-ping-error">Enter at least 2 characters for the shop or business name.</small>}<label>Deal type<select value={dealKind} onChange={e => setDealKind(e.target.value as DealKind)}>{DEAL_KINDS.map(kind => <option key={kind} value={kind}>{DEAL_KIND_LABEL[kind]}</option>)}</select></label></section>}
      {ping.category === "marketplace" && <section className="edit-ping-special"><label className="edit-ping-listing-type-label">Listing type</label><div className="edit-ping-market-types">{MARKETPLACE_LISTING_TYPE_ORDER.map(value => { const item=MARKETPLACE_LISTING_DEFINITIONS[value]; return <button type="button" key={value} className={marketplaceListingType === value ? "selected" : ""} onClick={() => chooseMarketplaceListingType(value)}><PingIcon name={item.icon} size={14}/>{item.label}</button>; })}</div>{!marketplaceListingDefinition && <small className="edit-ping-legacy-note">This older Marketplace listing uses a legacy combination. Choose a listing type only if you want to convert it.</small>}<div className="edit-ping-price"><label>{marketplaceListingDefinition?.priceFieldLabel || (marketplaceIntent === "rent" ? "£/month" : "Price")}<div><span>£</span><input inputMode="decimal" value={marketplacePrice} onChange={e => setMarketplacePrice(e.target.value.replace(/[^0-9.]/g,""))} placeholder="Optional"/></div></label></div>{!priceValid && <small className="edit-ping-error">Enter a price between £0 and £999,999,999.99.</small>}<label>External listing link<input type="url" value={marketplaceUrl} onChange={e => setMarketplaceUrl(e.target.value)} placeholder="https://..." maxLength={500}/></label>{!urlValid && <small className="edit-ping-error">Link must start with http:// or https://</small>}</section>}

...
export default function Phase22StorageChoice() {
  ...
  return (
    <aside className="phase22-storage-choice" aria-label="Browser storage choice">
      ...
      <style jsx global>{`
        .phase22-storage-choice{position:fixed;left:50%;bottom:max(14px,env(safe-area-inset-bottom));transform:translateX(-50%);z-index:1200;box-sizing:border-box;width:min(calc(100% - 28px),430px);max-height:calc(100dvh - max(14px,env(safe-area-inset-top)) - max(14px,env(safe-area-inset-bottom)));background:linear-gradient(150deg,rgba(9,31,52,.98),rgba(13,48,72,.98));color:#f7fbff;border:1px solid rgba(124,211,246,.24);border-radius:22px;padding:15px 16px;box-shadow:0 20px 60px rgba(4,21,36,.34);display:grid;gap:12px;backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px)}body[data-ping-global-nav-active="true"] .phase22-storage-choice{bottom:calc(var(--pd-tabbar-total,82px) + 14px);max-height:calc(100dvh - var(--pd-tabbar-total,82px) - max(14px,var(--pd-safe-top,env(safe-area-inset-top))) - 28px)}.phase22-storage-choice strong{font-size:14px;letter-spacing:-.01em}.phase22-storage-choice p{margin:5px 0 7px;color:#c7dbe8;font-size:11px;line-height:1.45}.phase22-storage-choice a{color:#7dd9f5;font-size:10px;font-weight:850;text-underline-offset:3px}.phase22-storage-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px}.phase22-storage-actions button{position:relative;z-index:1;min-height:44px;border:1px solid rgba(183,214,229,.34);background:rgba(255,255,255,.04);color:#f7fbff;border-radius:13px;padding:10px 8px;font-size:11px;font-weight:900;touch-action:manipulation}.phase22-storage-actions button.allow{background:linear-gradient(135deg,#1687b8,#1faac7);color:#fff;border-color:rgba(125,217,245,.44);box-shadow:0 8px 22px rgba(22,135,184,.2)}.phase22-storage-actions button:focus-visible{outline:3px solid rgba(125,217,245,.38);outline-offset:2px}@media(max-width:520px){.phase22-storage-choice{width:calc(100% - 20px);border-radius:20px;padding:14px}.phase22-storage-choice p{font-size:10.5px}.phase22-storage-actions button{min-height:44px;padding:9px 7px}}@media(max-height:700px){body[data-ping-global-nav-active="true"] .phase22-storage-choice{bottom:calc(var(--pd-tabbar-total,82px) + 10px)}.phase22-storage-choice{padding:12px 14px;gap:9px}.phase22-storage-choice p{margin:4px 0 5px}.phase22-storage-actions button{min-height:44px}}
      `}</style>
    </aside>
  );
}

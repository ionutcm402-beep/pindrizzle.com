...
function money(pence: number | null) { return `£${((pence || 0) / 100).toFixed(2)}`; }
...
export default function Phase9CheckoutPanel() {
  ...
  const load = useCallback(async () => {
    ...
    if (!sessionData.session) { setRequests([]); return; }
    const { data, error } = await supabase.rpc("my_promotion_requests");
    if (error) { console.error("Promotion payment list failed", error); return; }
  ...
  const verifyReturnedPayment = useCallback(async (sessionId: string) => {
    ...
    try {
      ...
      if (!token) throw new Error("Please sign in again so Pindrizzle can verify the payment.");
      const response = await fetch(pindrizzleApiUrl("/api/stripe/verify"), {
        method: "POST",
  ...
  const pay = async (promotionId: string) => {
    setLoadingId(promotionId); setMessage(""); setSuccess("");
    try {
      const supabase = createClient();
      const { data } = await supabase.auth.getSession();
      const token = data.session?.access_token;
      if (!token) throw new Error("Please sign in again before paying.");
      const response = await fetch(pindrizzleApiUrl("/api/stripe/checkout"), {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
  ...
  return createPortal(
    <section className="phase9-payment-panel">
      <div className="phase9-payment-heading"><div><strong>Payment</strong><span>Only approved promotions can be paid.</span></div><b>{requests.length}</b></div>
      {success && <div className="phase9-payment-success">✓ {success}</div>}
      {message && <div className="phase9-payment-error">{message}</div>}
      {requests.map((request) => <article key={request.promotion_id}>
        <div className="phase9-payment-top"><span>Approved · awaiting payment</span><strong>{money(request.quoted_price_pence)}</strong></div>
        <h3>{request.ping_title}</h3><p>{request.sponsor_name} · {radiusLabel(request.target_radius_meters)} · {request.duration_hours}h</p>
        <button type="button" disabled={loadingId === request.promotion_id} onClick={() => void pay(request.promotion_id)}>{loadingId === request.promotion_id ? "Opening secure checkout…" : `Pay ${money(request.quoted_price_pence)} securely`}</button>
        <small>Stripe checkout · no card details are handled by Pindrizzle.</small>
      </article>)}
      <style jsx global>{`

...
export default function Phase7ContributorContext() {
  ...
  useEffect(() => {
    if (!pingId) { setContext(null); return; }
    let cancelled = false;
    const load = async () => {
      const { data, error } = await createClient().rpc("ping_author_profile_context", { target_ping_id: pingId });
      if (cancelled) return;
      if (error) { console.error("Contributor context failed", error); setContext(null); return; }
  ...
  useEffect(() => {
    const ensureHost = () => {
      ...
      if (!sheet || !trust) { setHost(null); return; }
  ...
  return createPortal(<>
    <div className="phase7-contributor-card" aria-label="Contributor profile">
      ...
      <div className="phase7-contributor-copy"><span>Contributor profile</span><strong>{context.display_name}</strong><small>{memberLabel(context.member_since)} · {context.reputation_level}</small></div>
      <div className="phase7-contributor-signals"><div><strong>{context.helpful_pings}</strong><span>Helpful earned</span></div><div><strong>{context.confirmations}</strong><span>Confirms earned</span></div></div>
      <a className="phase7-profile-link" href={`/profile?id=${encodeURIComponent(context.profile_id)}`}>View profile <span>›</span></a>
    </div>
    <style jsx global>{`

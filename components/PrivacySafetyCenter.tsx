...
export default function PrivacySafetyCenter() {
  ...
  const loadSafetyData = useCallback(async () => {
    ...
    try {
      ...
      if (hiddenIds.length) {
        ...
        setHiddenPings(hideRows.map((row) => ({ id: row.ping_id, title: titles.get(row.ping_id) || "Reported Ping", createdAt: row.created_at })));
      } else {
        setHiddenPings([]);
      }
  ...
  const unblock = async (blockedId: string) => {
    ...
    try {
      ...
      setMessage("User unblocked. Their live Pings can appear again.");
  ...
  const unhide = async (pingId: string) => {
    ...
    try {
      ...
      setMessage("Ping unhidden. Your report remains submitted for review.");
      window.dispatchEvent(new CustomEvent("ping:community-changed", { detail: { kind: "safety" } }));
    } catch (error) {
      console.error("Unhide failed", error);
      setMessage("That Ping could not be unhidden right now.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="phase5-privacy-backdrop" role="dialog" aria-modal="true" aria-label="Privacy and safety" onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}>
      <section className="phase5-privacy-sheet">
        ...
        <div className="phase7-safety-note"><strong>Location privacy</strong><p>Ping stores only an approximate neighbourhood-scale point for public Pings. Your browser’s original GPS coordinate is not published as the Ping location.</p></div>

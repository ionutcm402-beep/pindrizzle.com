  ...
  const loadSummary = useCallback(async () => {
    ...
    try {
      ...
      if (row) {
        ...
        try {
          sessionStorage.setItem(SESSION_KEY, JSON.stringify({ userId, savedAt: Date.now(), summary: normalized } satisfies StoredSummary));
        } catch {}
      }
      setReady(true);
    } catch (error) {
  ...
  return createPortal(
    <section className={`phase8-return-card ${total > 0 ? "has-updates" : "is-caught-up"}`} aria-label="Since your last visit">
      <div className="phase8-return-top">
        ...
        <div className="phase8-return-mark">↻</div>
      </div>
      ...
      {!summary.first_visit && total > 0 && <button type="button" onClick={scrollToUpdates}>See what’s nearby ↓</button>}
      ...
      <style jsx>{`
        ...
        .phase8-return-top{display:grid;grid-template-columns:1fr 38px;gap:10px;align-items:start}.phase8-kicker{font-size:8px;font-weight:950;letter-spacing:.11em;color:#6b796e}.phase8-return-card h2{margin:5px 0 4px;font-size:17px;line-height:1.15;letter-spacing:-.35px}.phase8-return-card p{margin:0;color:#6e796f;font-size:10px;line-height:1.45}.phase8-return-mark{width:36px;height:36px;border-radius:13px;background:#dcebd5;display:grid;place-items:center;font-size:18px;font-weight:900;color:#416445}.phase8-return-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-top:13px}.phase8-return-grid div{padding:9px 4px;border-radius:13px;background:rgba(255,255,255,.72);text-align:center}.phase8-return-grid strong{display:block;font-size:17px}.phase8-return-grid span{display:block;margin-top:2px;color:#758078;font-size:7px;font-weight:850}.phase8-return-card button{width:100%;margin-top:10px;border:0;border-radius:12px;background:#1f3424;color:white;padding:10px;font-size:9px;font-weight:900}
      `}</style>
    </section>,
    host,
  );

...
export default function Phase25PrimaryNavigationBridge() {
  ...
  const [createStarting, setCreateStarting] = useState(false);
  ...
  return <>
    {showCreate && <button type="button" className="ping-global-create" onClick={createPin} aria-label="Drop a pin" aria-busy={createStarting} disabled={createStarting}><PingIcon name="plus" size={18}/><span>Drop a pin</span></button>}
    ...
    <style jsx global>{`
      ...
      .ping-global-create{position:fixed;z-index:151;right:max(20px,calc((100vw - 444px)/2 + 12px));bottom:max(92px,calc(80px + env(safe-area-inset-bottom)));min-height:44px;display:inline-flex;align-items:center;justify-content:center;gap:7px;padding:0 16px;border:0;border-radius:999px;background:var(--ping-ink);color:#fff;box-shadow:0 12px 28px rgba(16,19,17,.18);font-size:11px;font-weight:780;cursor:pointer}.ping-global-create:active{transform:scale(.97)}.ping-global-create[aria-busy="true"]{opacity:.72;cursor:wait}.ping-global-create[aria-busy="true"]::after{content:"";width:12px;height:12px;border:2px solid rgba(255,255,255,.45);border-top-color:#fff;border-radius:50%;animation:pingCreateSpin .7s linear infinite}@keyframes pingCreateSpin{to{transform:rotate(360deg)}}
      ...
      @media(prefers-reduced-motion:reduce){.ping-global-create[aria-busy="true"]::after{animation:none}}
    `}</style>
  </>;
}

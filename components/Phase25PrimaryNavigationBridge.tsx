...
const APP_PATHS = new Set(["/", "/map", "/my-pings", "/following", "/alerts", "/notifications", "/you"]);
  ...
  return (
    <>
      ...
      <style jsx global>{`
        ...
        .ping-global-nav-item{position:relative;min-width:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;border-radius:13px;color:#7d847f;text-decoration:none;font-size:9px;font-weight:720;line-height:1;letter-spacing:-.01em}.ping-global-nav-item svg{display:block;color:currentColor;flex:0 0 auto}.ping-global-nav-item span{display:block;white-space:nowrap}.ping-global-nav-item.active{color:var(--ping-accent-ink)}.ping-global-nav-item:active{background:var(--ping-surface-soft)}.ping-global-nav-item .ping-global-unread{position:absolute;top:5px;left:calc(50% + 7px);min-width:16px;height:16px;display:grid;place-items:center;border:2px solid #fff;border-radius:999px;background:var(--ping-danger);color:#fff;padding:0 3px;font-size:7px;font-style:normal;font-weight:850;line-height:1}
        .ping-global-compose{position:fixed;z-index:151;right:max(20px,calc((100vw - 444px)/2 + 12px));bottom:max(92px,calc(80px + env(safe-area-inset-bottom)));min-height:44px;display:inline-flex;align-items:center;justify-content:center;gap:7px;padding:0 16px;border:0;border-radius:999px;background:var(--ping-ink);color:#fff;box-shadow:0 12px 28px rgba(16,19,17,.18);font-size:11px;font-weight:780;cursor:pointer}.ping-global-compose svg{display:block}.ping-global-compose:active{transform:scale(.97)}
        @media(max-width:350px){.ping-global-nav{width:calc(100% - 18px);padding-left:4px;padding-right:4px}.ping-global-nav-item{font-size:8px}.ping-global-compose{right:14px}}@media(min-width:521px){.ping-global-compose{right:calc(50% - 210px)}}@media(prefers-reduced-motion:reduce){.ping-global-compose:active{transform:none}}
      `}</style>
    </>
  );
}

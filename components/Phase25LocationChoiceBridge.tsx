  ...
  useEffect(() => {
    ...
    const updateCopy = () => {
      ...
      document.querySelectorAll<HTMLElement>(".map-v3-location small").forEach((node) => {
        if (node.textContent?.includes("never publishes your exact")) {
          node.textContent = "One location permission powers Feed and Map. When posting, you choose Private area or Exact point.";
        }
      });

...
async function waitForRegistration() {
  return new Promise<string>((resolve, reject) => {
    let settled = false;
    let successHandle: { remove: () => Promise<void> } | null = null;
    ...
    const finish = (error: Error | null, token?: string) => {
      ...
      if (successHandle) void successHandle.remove();
      if (errorHandle) void errorHandle.remove();
      if (error) reject(error);
      else resolve(token || "");
    };

    const timer = window.setTimeout(() => finish(new Error("Native push registration timed out.")), REGISTRATION_TIMEOUT_MS);

    void PushNotifications.addListener("registration", (token: Token) => finish(null, token.value)).then((handle) => { successHandle = handle; });
    void PushNotifications.addListener("registrationError", (event) => finish(new Error(event.error || "Native push registration failed."))).then((handle) => { errorHandle = handle; });
    void PushNotifications.register().catch((error) => finish(error instanceof Error ? error : new Error("Native push registration failed.")));
  });
}
...
export async function disableNativePushDevice() {
  ...
  try { await PushNotifications.unregister(); } catch (error) { console.warn("Native push unregister failed", error); }
  forgetToken();
}
